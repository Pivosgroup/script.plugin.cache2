...
class StorageServer():
    def __init__(self, table=None, timeout=24, instance=False):
        ...
        self.settings = self.xbmcaddon.Addon(id='script.plugin.cache2')
        ...
        self.path = os.path.join(self.path, 'PluginCache2.db')
    ...
    def _sock_init(self, check_stale=False):
        ...
        if not self.socket or check_stale:
            ...
            if self.platform == "win32" or xbmc.getCondVisibility('system.platform.android'):
                ...
                self._log("POSIX", 4)
                self.socket = os.path.join(self.xbmc.translatePath('special://temp/').decode("utf-8"), 'commoncache2.socket')
...
### EXTERNAL FUNCTIONS ###
    ...
    def set_plus(self, name, data):
        self._log(name, 1)
        if self._connect() and self.table:
            temp = repr({"action": "set_plus", "table": self.table, "name": name, "data_size": 1})
            res = self._send(self.soccon, temp)
            self._log(u"GOT " + repr(res), 3)
...
def checkInstanceMode():
    ...
    settings = xbmcaddon.Addon(id='script.plugin.cache2')
